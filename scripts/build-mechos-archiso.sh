...
# Dedicated recovery boot entry routes straight to Recovery Center.
...
STYLE = """
QWidget { background:#09060f; color:#f6edff; font-family:Sans Serif; }
QFrame#panel { background:#15101f; border:1px solid #39254a; border-radius:14px; }
QPushButton { background:#251533; border:1px solid #704192; border-radius:10px;
              padding:13px 17px; text-align:left; font-weight:650; }
QPushButton:hover { background:#352047; border-color:#a75ed9; }
QPlainTextEdit { background:#0b0810; border:1px solid #39254a; border-radius:8px; padding:8px; }
"""

class Setup(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MechOS Setup")
    ...
    def build_ui(self):
        ...
        top = QHBoxLayout()
        logo = QLabel()
        path = "/usr/share/mechos/branding/mechos-logo.png"
        if os.path.exists(path):
            pix = QPixmap(path).scaledToHeight(80, Qt.TransformationMode.SmoothTransformation)
            logo.setPixmap(pix)
            top.addWidget(logo)

        titlebox = QVBoxLayout()
        title = QLabel("MECHOS SETUP")
        title.setFont(QFont("Sans Serif", 27, QFont.Weight.Bold))
        titlebox.addWidget(title)
        sub = QLabel("Gaming • Creator • Desktop • Recovery")
        sub.setStyleSheet("color:#baa8c6;")
        ...
        heading = QLabel("LIVE SETUP")
        heading.setFont(QFont("Sans Serif", 16, QFont.Weight.Bold))
        al.addWidget(heading)

        for label, fn in [
            ("Install MechOS", self.install),
            ("Reinstall MechOS — Keep Home", self.preserve_home),
            ("Recovery & Repair", self.recovery),
            ("Run Hardware Scan", self.refresh_hw),
            ("View Install Logs", self.logs),
            ("Continue Live Session", self.close),
        ]:
            b = QPushButton(label)
            b.clicked.connect(fn)
            al.addWidget(b)
        ...
        il = QVBoxLayout(info)
        h = QLabel("HARDWARE PRECHECK")
        h.setFont(QFont("Sans Serif", 16, QFont.Weight.Bold))
        il.addWidget(h)
        self.hw = QPlainTextEdit()
        self.hw.setReadOnly(True)
        il.addWidget(self.hw, 1)
        body.addWidget(info, 2)

        outer.addLayout(body, 1)

        safe = QLabel(
            "MechOS never hides Archinstall's final disk summary. Normal installation and "
            "preserve-home reinstall both require an explicit storage confirmation before "
            "any formatting operation."
        )
        safe.setWordWrap(True)
        safe.setStyleSheet("color:#a994b7;")
    ...
    def refresh_hw(self):
        try:
            ...
            self.hw.setPlainText(out)
        except Exception as e:
            self.hw.setPlainText(str(e))

    def install(self):
        self.refresh_hw()
        if QMessageBox.question(
            self, "Install MechOS",
            "Open the guided MechOS installer?\n\n"
            "You will still choose the disk, filesystem, user account and bootloader, "
            "and Archinstall will show its final disk summary before changes are made."
        ) != QMessageBox.StandardButton.Yes:
            return
        subprocess.Popen(["konsole", "-e", "sudo", "/usr/local/bin/mechos-install", "--terminal"])

    def preserve_home(self):
        if QMessageBox.question(
            self, "Reinstall — Keep Home",
            "Open the preserve-home assistant?\n\n"
            "MechOS will scan the current layout and tell you which /home storage to preserve. "
            "The final disk setup remains manual so /home is never silently formatted."
        ) != QMessageBox.StandardButton.Yes:
            return
