...
log() { printf '[MechOS v0.2.2] %s\n' "$*"; }
fail() { printf '[MechOS v0.2.2] ERROR: %s\n' "$*" >&2; exit 1; }
...
class SetupCenter(QMainWindow):
    def __init__(self):
        ...
        layout.addStretch(1)
        foot = QLabel("MechOS v0.2.2 Alpha • Live environment")
...
# ---------------------------------------------------------------------------
# v0.2.2 installed desktop-environment repair
# ---------------------------------------------------------------------------
if [ -d "$PAYLOAD" ] && [ -f "$PAYLOAD/mechos-postinstall-target" ]; then
  cp -f "$BIN/mechos-desktop-check" "$PAYLOAD/mechos-desktop-check"
  chmod 644 "$PAYLOAD/mechos-desktop-check"

  if ! grep -Fq 'MECHOS_V0_2_2_DESKTOP_POSTINSTALL' "$PAYLOAD/mechos-postinstall-target"; then
    cat >> "$PAYLOAD/mechos-postinstall-target" <<'POSTEOF'

# MECHOS_V0_2_2_DESKTOP_POSTINSTALL
echo "[MechOS] Installing and verifying KDE Plasma desktop..."
...
echo "[MechOS] Configuring desktop for user: $TARGET_USER"
...
if [ ! -x /usr/bin/startplasma-wayland ]; then
  echo "[MechOS] ERROR: startplasma-wayland is missing." >&2
  exit 63
fi

if [ ! -f /usr/share/wayland-sessions/plasma.desktop ]; then
  echo "[MechOS] ERROR: plasma.desktop is missing." >&2
  exit 64
fi

if [ ! -x /usr/local/bin/mechos-gaming-session ]; then
  echo "[MechOS] ERROR: mechos-gaming-session is missing." >&2
  exit 65
fi
...
if command -v curl >/dev/null 2>&1; then
  curl -fsS "http://127.0.0.1:45811/mechos-desktop-check" \
    -o /usr/local/bin/mechos-desktop-check || true
  chmod 755 /usr/local/bin/mechos-desktop-check 2>/dev/null || true
fi

mkdir -p /var/lib/mechos /var/log
DESKTOP_LOG="/var/log/mechos-desktop-install.log"
{
  echo "MechOS desktop install verification"
  echo "Date: $(date -Is)"
...
echo "[MechOS] KDE Plasma desktop installation verified."
POSTEOF
    chmod 755 "$PAYLOAD/mechos-postinstall-target"
  fi
fi
