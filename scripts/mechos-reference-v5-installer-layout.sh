...
# Ensure the final installer owns the helpers needed by the reference layout.
if 'import json\n' not in text:
...
replacement=r'''class Installer(QMainWindow):
    ...
    def build_ui(self):
        ...
        # Left numbered navigation, matching the approved reference.
        ...
        driveicon=QLabel('▣'); driveicon.setStyleSheet('font-size:34px;color:#c66dff'); targetrow.addWidget(driveicon)
        ...
        change=QPushButton('Change Drive'); change.clicked.connect(self.open_partition_selector); targetrow.addWidget(change)
        tl.addLayout(targetrow); center.addWidget(target)

        options=self.panel(); ol=QVBoxLayout(options); ol.setContentsMargins(18,14,18,14); ol.setSpacing(9)
        ol.addWidget(self.section('CHOOSE INSTALLATION OPTION'))
        ...
        right.addWidget(overview); right.addStretch()
        ...
        install=QPushButton('Install Now\nStart installation'); install.setObjectName('primary'); install.clicked.connect(self.install); footer.addWidget(install)
    ...
    def install_card(self,mode,title,desc,detail):
        ...
        rb.toggled.connect(lambda checked,m=mode,c=check:self.set_mode(m,checked,c))
        p.mousePressEvent=lambda _event,b=rb: b.setChecked(True)
        self.mode_cards[mode]=p
        return p
    ...
    def set_mode(self,mode,checked,check=None):
        ...
        self.ov_mode.setText(names[mode])
        if mode=='clean':
            self.warning_text.setText('Clean Install erases only the selected whole disk after a final MechOS confirmation.')
            self.ov_partition.setText('GPT • BIOS + UEFI'); self.ov_fs.setText('Btrfs')
        elif mode=='keep':
            self.warning_text.setText('/home, accounts, passwords, network identity and completed first-boot state are preserved by the Live update/reinstall path.')
            self.ov_partition.setText('Existing layout preserved'); self.ov_fs.setText('Existing filesystem')
        elif mode=='custom':
            self.warning_text.setText('Custom Install opens the read-only partition selector and manual partitioning. Nothing is formatted silently.')
            self.ov_partition.setText('Manual'); self.ov_fs.setText('User selected')
        else:
            self.warning_text.setText('Alongside mode scans existing systems read-only and guides a dual-boot layout. Resizing/formatting is never automatic.')
            self.ov_partition.setText('Guided dual boot'); self.ov_fs.setText('User confirmed')
    ...
    def refresh_target_labels(self):
        if self.selected_disk:
            ...
            self.drive_name.setText('No install target selected'); self.drive_detail.setText('Choose a target drive before Clean Install.'); self.drive_space.setText(''); self.ov_drive.setText('None')
    ...
    def sync_selection_from_disk(self):
        ...
        try:
            ...
            payload={'kind':'disk','path':self.selected_disk,'disk':self.selected_disk,'size_bytes':size,'fstype':'','label':''}
            self.SELECTION.write_text(json.dumps(payload,indent=2),encoding='utf-8')
    ...
    def open_partition_selector(self):
        subprocess.Popen(['/usr/local/bin/mechos-partition-selector'])

    def nav_selected(self,row):
        if row==1:
            subprocess.Popen(['konsole','-e','bash','-lc',"/usr/local/bin/mechos-hardware-scan; echo; read -rp 'Press Enter to close…'"])
        elif row==2: self.open_partition_selector()
        elif row==3: self.mode_buttons[self.install_mode].setFocus()
        elif row==4: self.recovery()
        elif row==5: self.recovery()
        elif row==6:
            self.mode_buttons['keep'].setChecked(True); self.install()
        elif row==7:
            subprocess.Popen(['konsole','-e','bash','-lc',"/usr/local/bin/mechos-recovery-helper logs; echo; read -rp 'Press Enter to close…'"])
        elif row==8: self.close()

    def install(self):
        if self.install_mode=='keep':
            subprocess.Popen(['konsole','-e','sudo','/usr/local/bin/mechos-live-update-keep-home']); return
        if self.install_mode=='alongside':
            subprocess.Popen(['konsole','-e','sudo','/usr/local/bin/mechos-alongside-assistant']); return
        if self.install_mode=='custom':
            # Selection remains read-only; manual partitioning owns destructive choices.
            subprocess.Popen(['/usr/local/bin/mechos-partition-selector'])
            QMessageBox.information(self,'Custom Install','Choose the target partition/disk. Then launch Custom Install again to continue into manual partitioning.')
            if self.SELECTION.is_file():
                subprocess.Popen(['konsole','-e','sudo','/usr/local/bin/mechos-install-selected-target'])
            return
        if not self.selected_disk:
            QMessageBox.warning(self,'MechOS Installer','Select a whole target disk before starting Clean Install.'); return
        self.sync_selection_from_disk()
