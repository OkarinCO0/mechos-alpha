...
marker='# MECHOS_REFERENCE_MECHSCOPE_EXACT_V1'
if marker in text:
    raise SystemExit(0)
...
# Reusable painted circular telemetry meter. It intentionally has no OpenGL or
# QtQuick dependency, so VirtualBox/VMware/QEMU render the same widget through
# the normal Qt raster path while physical systems keep their accelerated path.
helper=r'''
...
class MechReferenceGauge(QWidget):
    ...
    def paintEvent(self,event):
        ...
        side=max(72,min(self.width(),self.height())-16)
        x=(self.width()-side)/2
        y=5
        rect=QRectF(x,y,side,side)
        base=QPen(QColor('#172039'),9)
        base.setCapStyle(_Qt.PenCapStyle.RoundCap)
        p.setPen(base)
        p.drawArc(rect,90*16,-360*16)
        if self.value is not None:
            active=QPen(QColor(self.accent),9)
            active.setCapStyle(_Qt.PenCapStyle.RoundCap)
            p.setPen(active)
        ...
        p.drawText(rect.adjusted(0,13,0,0),_Qt.AlignmentFlag.AlignHCenter|_Qt.AlignmentFlag.AlignTop,self.title)
        f=QFont(); f.setPointSize(17); f.setBold(True); p.setFont(f)
        value='--' if self.value is None else f'{self.value}%'
        p.drawText(rect,_Qt.AlignmentFlag.AlignCenter,value)
        p.setPen(QColor('#8fa1ba'))
        f=QFont(); f.setPointSize(7); p.setFont(f)
        p.drawText(rect.adjusted(0,0,0,-12),_Qt.AlignmentFlag.AlignHCenter|_Qt.AlignmentFlag.AlignBottom,self.detail)
...
if class_pos < 0:
    raise SystemExit('[MechOS MechScope Exact Reference] MechScope class insertion point missing')
...
build=r'''    def build_ui(self):
        ...
        # Reference header: logo/brand left, surface title centered, live network
        # and clock on the right. The same header remains visible in VM mode.
        top=self.panel('top'); top.setMinimumHeight(62)
        tl=QHBoxLayout(top); tl.setContentsMargins(24,9,24,9); tl.setSpacing(14)
        brand=QLabel('◉  MECHOS'); brand.setObjectName('brand'); tl.addWidget(brand)
        tl.addStretch(1)
        scope=QLabel('MECHSCOPE 2.0'); scope.setObjectName('scope'); scope.setStyleSheet('font-size:28px;font-weight:900;letter-spacing:4px'); tl.addWidget(scope)
        tl.addStretch(1)
        ...
        # LEFT / MAIN REFERENCE COLUMN -------------------------------------------------
        main=QVBoxLayout(); main.setSpacing(10)
        hero=self.panel('hero'); hero.setMinimumHeight(286)
        ...
        sub=QLabel('Your unified command center for gaming,\nperformance, and creation.'); sub.setObjectName('muted'); sub.setStyleSheet('font-size:16px'); copy.addWidget(sub)
        copy.addStretch(1)
        ...
        if not pix.isNull():
            art.setPixmap(pix.scaled(620,265,Qt.AspectRatioMode.KeepAspectRatio,Qt.TransformationMode.SmoothTransformation))
        ...
        modes=self.panel(); ml=QVBoxLayout(modes); ml.setContentsMargins(14,10,14,12); ml.setSpacing(8)
        mt=QLabel('QUICK MODES'); mt.setObjectName('section'); ml.addWidget(mt)
        ...
        for label,detail,fn,active in mode_defs:
        ...
        for label,detail,fn in tools:
            b=self.focus_button(QPushButton(label+'\n'+detail)); b.setMinimumHeight(50); b.clicked.connect(fn); tsl.addWidget(b,1)
        main.addWidget(toolstrip)
        body.addLayout(main,5)

        # MIDDLE REFERENCE COLUMN ------------------------------------------------------
        middle=QVBoxLayout(); middle.setSpacing(10)
        status=self.panel(); sl=QVBoxLayout(status); sl.setContentsMargins(14,12,14,12); sl.setSpacing(8)
        sh=QLabel('SYSTEM STATUS'); sh.setObjectName('section'); sl.addWidget(sh)
        gauges=QHBoxLayout(); gauges.setSpacing(4)
        self.cpu_gauge=MechReferenceGauge('CPU',None,'Load','#49deff')
        self.ram_gauge=MechReferenceGauge('RAM',None,'Used','#a85cff')
        self.disk_gauge=MechReferenceGauge('DISK',None,'Used','#258cff')
        for gauge in (self.cpu_gauge,self.ram_gauge,self.disk_gauge): gauges.addWidget(gauge,1)
        sl.addLayout(gauges)
        self.stats_label=QLabel(); self.stats_label.hide(); sl.addWidget(self.stats_label)
        ...
        quick=self.panel(); ql=QVBoxLayout(quick); ql.setContentsMargins(14,12,14,12); ql.setSpacing(8)
        qt=QLabel('QUICK ACTIONS'); qt.setObjectName('section'); ql.addWidget(qt)
        ...
        ql.addStretch(1); middle.addWidget(quick,1)
        body.addLayout(middle,2)

        # RIGHT REFERENCE COLUMN -------------------------------------------------------
        side=QVBoxLayout(); side.setSpacing(10)
        launchers=self.panel(); ll=QVBoxLayout(launchers); ll.setContentsMargins(14,12,14,12); ll.setSpacing(8)
        lt=QLabel('LAUNCHERS'); lt.setObjectName('section'); ll.addWidget(lt)
        launcher_actions=[
            ('Steam',self.open_steam),
            ('Unified Store',self.open_store),
            ('Performance Center',lambda:spawn(['/usr/local/bin/mechos-performance-center'])),
        ...
        power=self.panel(); pl=QVBoxLayout(power); pl.setContentsMargins(14,12,14,12); pl.setSpacing(10)
        pt=QLabel('SYSTEM'); pt.setObjectName('section'); pl.addWidget(pt)
        ...
        shutdown=self.focus_button(QPushButton('Shut Down')); shutdown.setObjectName('danger'); shutdown.setMinimumHeight(68); shutdown.clicked.connect(lambda:spawn(['systemctl','poweroff'])); pl.addWidget(shutdown)
...
# Direct Plasma VM sessions must occupy the same full-screen canvas as the
# physical Gamescope path. Preserve this even if the generic v5 styler changed
# the final show call to showMaximized().
text=text.replace('w.showMaximized()','w.showFullScreen()')
text=text.replace('win.showMaximized()','win.showFullScreen()')
...
PYTHONDONTWRITEBYTECODE=1 python3 -m py_compile "$FILE" \
  || fail "MechScope Python failed after exact-reference layout"
grep -Fq 'MECHOS_REFERENCE_MECHSCOPE_EXACT_V1_HOME' "$FILE" \
  || fail "exact MechScope home marker missing"
grep -Fq 'class MechReferenceGauge' "$FILE" \
  || fail "reference circular gauge widget missing"
grep -Fq 'Creator Store' "$FILE" \
  || fail "bottom reference tool strip is incomplete"
grep -Fq 'LAUNCHERS' "$FILE" \
  || fail "right launcher column is missing"
grep -Fq 'QUICK ACTIONS' "$FILE" \
  || fail "middle quick-actions column is missing"
grep -Fq 'showFullScreen()' "$FILE" \
  || fail "MechScope is not forced full-screen for the VM/physical reference canvas"
